...
import { getReservationDays } from '@/services/reservation.api';
...
function getYearMonth(sp: ReturnType<typeof useSearchParams>) {
  const now = new Date();
  const raw = sp.get('ym') || `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
  const [y, m] = raw.split('-').map(Number);
  const year = Number.isFinite(y) ? y : now.getFullYear();
  const month1 = Number.isFinite(m) ? m : now.getMonth() + 1;
  return { year, monthIdx: month1 - 1, label: `${year}.${String(month1).padStart(2, '0')}` };
}

const getErr = (e: unknown) => e instanceof Error ? e.message : '날짜 조회 실패';
...
export default function ConsultDaysPage() {
  ...
  const [availableDays, setAvailableDays] = React.useState<Set<number>>(new Set());
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const days = await getReservationDays({ vendorId, year, month: month1 });
  ...
  return (
    <ReservationLayout
      title="예약하기"
      step={2}
      headline={<>상담 받고싶은 날짜를 <br />선택해 주세요.</>}
      mode="single"
      primaryText="다음"
      active={!!selectedDay}
      onPrimary={() => {
        if (!selectedDay || !vendorId) return;
        const date = `${year}-${String(month1).padStart(2, '0')}-${String(selectedDay).padStart(2, '0')}`;
    ...
    >
      ...
      {loading && <div className="mb-2 text-sm text-text--secondary">달력을 불러오는 중…</div>}
      {error && <div className="mb-2 text-sm text-red-500">오류: {error}</div>}

      <div className="grid grid-cols-7 gap-3 mb-2">
        {weekdayLabels.map((w) => (
          <div key={w} className="text-center text-sm text-text-secondary">{w}</div>
        ))}
      </div>

      <div className="grid grid-cols-7 gap-3">
        {cells.map((d, idx) => {
          if (d === null) return <div key={`empty-${idx}`} className="w-10 h-10" />;
          ...
          return (
            <div key={d} className="flex justify-center">
              <CalendarDay
                ...
                day={
                  <span className={isSelected ? 'text-red-500' : idx % 7 === 6 ? 'text-red-500' : ''}>
                    {d}
                  </span>
                }
